...
pub fn App() -> impl IntoView {
    ...
    let client: Rc<
        RefCell<Option<SplitSink<WebSocket, gloo_net::websocket::Message>>>
    > = Default::default();
    ...
    create_effect(move |_| {
        ...
        let connection = WebSocket::open(&format!("{ws_url}")).expect(
        ...
        spawn_local(async move {
            while let Some(msg) = recv.next().await {
                match msg {
                    ...
                    _ => {
                        set_is_loading.update(|n| *n=false);
                        break;
                    }
                }
            }
    ...
    let send = create_action(move |new_message: &String| {
        ...
        set_is_loading.update(|n| *n=true);
    ...
    view! {
        ...
        <div class="h-screen w-full flex flex-col justify-center items-center">
            <ChatArea conversation is_loading/>
            <TypeArea send is_loading/>
        </div>
    }
}
